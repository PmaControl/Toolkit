...
function get_port()
{
    ...
    for ((i = ${1}; i <= ${2}; i++)); do
    	result=$(nmap -p "$i" localhost | grep "$i/tcp")
		if [[ $result == *open* ]]; then
			echo ""
			...
			echo ""
		    #echo "Impossible de déterminer l'état du port $i."
		fi
	done
}
...
for ver in "${version[@]}"; do
	...
	docker network create --subnet "${part_ip}.0/24" "$network"

	domain_id=$(crc32-string "${part_ip}")

	for i in 1 2 3; do

		dir="/srv/galera/${ver}/node${i}"
		mkdir -p "$dir"
		chown 999:999 "${dir}"
		...
		cat <<EOF > "${dir}/galera.cnf"
[mysqld]
wsrep_cluster_address = "gcomm://${part_ip}.1,${part_ip}.2,${part_ip}.3"
wsrep_cluster_name    = "galera-cluster-${ver_sep}"
