...
function getProxy()
{
  cat /etc/dnf/*.conf 2>/dev/null |{ grep -E 'proxy=' | grep -Eo 'https?://.*([0-9]+|/)' || true;}
}
...
function purge {
 ...
 rm -rf /etc/mysql/*
dnf -y remove $(rpm -qa | grep mariadb )
dnf -y remove $(rpm -qa | grep mysql)
dnf -y remove $(rpm -qa | grep percona)
dnf -y autoremove
dnf clean all
}
...
#dnf -y install bsdmainutils
dnf -y install findutils coreutils util-linux
...
#mytest dnf -qq -y install software-properties-common
...
if [ $VERSION = "galera57" ]; then
        ...
        mytest dnf -y install mysql-wsrep-server-5.7 galera-3 > /dev/null
else
        ...
        echo "There will be install MariaDB !!!!"
        mytest dnf -y install MariaDB-server MariaDB-client MariaDB-backup MariaDB-common
...
cp -pr /etc/my.cnf.d/client.cnf /etc/my.cnf.d/debian.cnf

chown mysql:mysql -R /etc/my.cnf.d/debian.cnf

cat > /etc/my.cnf.d/debian.cnf << EOF
...
# mytest dnf install -y netcat tar socat lsof > /dev/null

mytest dnf install -y tar socat lsof > /dev/null
...
yum -y install https://github.com/mydumper/mydumper/releases/download/${release}/mydumper-${release:1}.el9.x86_64.rpm
#mytest dnf -y install mydumper > /dev/null


#vim 

mytest dnf -y install vim > /dev/null
...
dnf install -y tree mlocate screen iftop htop curl git unzip atop nmap > /dev/null
